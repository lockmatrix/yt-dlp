...
class XiaoYuZhouIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        upload_datetime = datetime.strptime(pubDateStr, "%Y-%m-%dT%H:%M:%S.%fZ") if pubDateStr else None
        ...
        podcast_url = f'https://www.xiaoyuzhoufm.com/podcast/{podcast_id}' if podcast_id else None

        podcast_user_list = traverse_obj(episode_info, ('podcast', 'podcasters', ...))

        formats = []
        if audio_url:
            ext = audio_url.rsplit('.', 1)[-1] if '.' in audio_url.split('/')[-1] else None
            formats.append({
                'url': audio_url,
                'vcodec': 'none',
                'ext': ext,
        ...
        return {
            ...
            'duration': duration,
            'timestamp': upload_datetime.timestamp() if upload_datetime else None,
